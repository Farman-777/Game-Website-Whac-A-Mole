...
import "./GameMouseLevel3.css";
import Confetti from "react-confetti";
import Swal from "sweetalert2";

const GameMouseLevel3 = () => {
  ...
  const handleHit = (index) => {
    if (index === mouseIndex) {
      ...
      if (showBR === "#D40032") {
        ...
        Swal.fire({
          title: "Game Over!",
          text: `You hit the mole on a pink background. Game Over!`,
          icon: "error",
          confirmButtonText: "OK",
        }).then(() => {
          wrongHitAudio.pause();
  ...
  return (
    <div>
      {gameStarted ? ( // Button is hidden when game is started
        <div></div>
      ) : (
        <button className="back-Button-hard" onClick={handleclickBack}>
          <FontAwesomeIcon icon={faArrowLeft} />
        </button>
      )}
      <div
        className="main-container-level3"
        style={
          gameStarted
            ? {
                marginTop: "0px",
      ...
      >
        <h1 className="game-title-level3">Whac A Mole</h1>
        <button
          type="button"
          className="start-button-level3"
        ...
        {gameStarted && (
          <div className="game-container-level3">
            <div className="level3-part-1">
              <div
                className="Inner-box-level3"
                style={{
            ...
            <div className="level3-part-1">
              <div
                className="Inner-box-level3"
                style={{
                  backgroundColor: mouseIndex === 9 ? `${showBR}` : "white",
                }}
                onClick={() => handleHit(9)}
              >
                Hit Me
              ...
              <div
                className="Inner-box-level3"
                style={{
                  backgroundColor: mouseIndex === 10 ? `${showBR}` : "white",
                }}
                onClick={() => handleHit(10)}
              >
                Hit Me
              </div>
              <div
                className="Inner-box-level3"
                style={{
                  backgroundColor: mouseIndex === 11 ? `${showBR}` : "white",
                }}
                onClick={() => handleHit(11)}
              >
                Hit Me
              </div>
            </div>

            <div className="level3-part-2">
...
export default GameMouseLevel3;
